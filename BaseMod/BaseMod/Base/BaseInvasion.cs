...
namespace ofDarkandBelow
{
	/*public class BaseInvasion
	{
		...
		public void OnWorldUpdate()
		{
			...
			undeadipedeCount = Redemption.BaseAI.GetNPCs(default(Vector2), GRealm.UNDEADIPEDE_TYPES[0], -1, null).Length + Redemption.BaseAI.GetNPCs(default(Vector2), GRealm.UNDEADIPEDE_TYPES[1], -1, null).Length;	
		...
		public void HandleHardMobSpawn(int type)
		{
            if (Main.netMode != 1 && IsHardMob(type))
			{
				...
				if(chat != null) Redemption.BaseUtility.Chat(chat, Redemption.BaseConstants.CHATCOLOR_PURPLE);				
			}	
		}
		...
		public static IDictionary<int, float> SpawnPool(IDictionary<int, float> startingPool, int x, int y, Player player)
		{
			...
			}else //normal zombies
			{
				...
				if (ZONEDEV || Redemption.BaseTile.LiquidCount(x, y, 25, 0) > 120) //floaty zombie
				{
