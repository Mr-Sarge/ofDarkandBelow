...
{
    public class HunchBack : ModNPC
    {
        public override void SetStaticDefaults()
        {
            DisplayName.SetDefault("HunchBack");
        ...
        public override void SetDefaults()
        {
            ...
            banner = npc.type;
            bannerItem = mod.ItemType("HunchBackBanner");
        }
        public override void HitEffect(int hitDirection, double damage)
        {
            if (npc.life <= 0)          //this make so when the npc has 0 life(dead) he will spawn this
            {
                Gore.NewGore(npc.position, npc.velocity, mod.GetGoreSlot("Gores/HunchGore1"), 1f);
                Gore.NewGore(npc.position, npc.velocity, mod.GetGoreSlot("Gores/HunchGore2"), 1f);
