...
{
    public class DragonLampTile : ModTile
    {
        public override void SetDefaults()
        {
            ...
            AddMapEntry(new Color(213, 202, 179), Language.GetText("Dragon Shrine Lamp"));
        ...
        {
            Item.NewItem(i * 16, j * 16, 32, 48, mod.ItemType("DragonLampItem"));
        }
        ...
        public override void PostDraw(int i, int j, SpriteBatch spriteBatch)
        {
            ...
            var flameTexture = mod.GetTexture("Tiles/DragonShrine/DragonLampTile_flame"); // We could also reuse Main.FlameTexture[] textures, but using our own texture is nice.

            ulong num190 = Main.TileFrameSeed ^ (ulong)((long)j << 32 | (long)(uint)i);
            // We can support different flames for different styles here: int style = Main.tile[j, i].frameY / 54;
            for (int c = 0; c < 7; c++)
            {
                float shakeX = Utils.RandomInt(ref num190, -10, 11) * 0.09f;
                float shakeY = Utils.RandomInt(ref num190, -10, 1) * 0.50f;
