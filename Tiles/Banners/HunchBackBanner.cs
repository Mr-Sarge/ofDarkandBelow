...
{
    public class HunchBackBanner : ModTile
    {
        ...
        {
            Item.NewItem(i * 16, j * 16, 16, 48, mod.ItemType("HunchBackBanner"));//this defines what to drop when this tile is destroyed
        }
        ...
        public override void NearbyEffects(int i, int j, bool closer)   //this make so the banner give an effect to nearby players
        {
            if (closer)          //so if a player is close to the banner
            {
                ...
                player.NPCBannerBuff[mod.NPCType("HunchBack")] = true;  // give to player the npcbannerBuff. for a specific npc. change NpcName to your npc name
