...
namespace ofDarkandBelow.Tiles.Banners     //We need this to basically indicate the folder where it is to be read from, so you the texture will load correctly
{
    public class CrimHunchBackBanner : ModTile
    {
        ...
        {
            Item.NewItem(i * 16, j * 16, 16, 48, mod.ItemType("CrimHunchBackBanner"));//this defines what to drop when this tile is destroyed
        }
        ...
        public override void NearbyEffects(int i, int j, bool closer)   //this make so the banner give an effect to nearby players
        {
            if (closer)          //so if a player is close to the banner
            {
                ...
                player.NPCBannerBuff[mod.NPCType("CrimHunchBack")] = true;  // give to player the npcbannerBuff. for a specific npc. change NpcName to your npc name
